...
OpenCMO 现在在项目工作区内内置了正式的报告能力：

- **战略报告** — 在一次完整扫描后生成，回答“这个项目是什么、当前优势和短板在哪里、竞品如何占位、CMO 级判断是什么”。
- **邮件复用** — 周报邮件直接复用已持久化的周期性人类版报告，而不是再拼一套独立快照模板。
- **周报** — 基于最近 7 天监控窗口生成，汇总 SEO、GEO、SERP、社区、审批等维度最重要的变化。
- **双版本输出** — 每份报告都会同时保存 **Human Readout** 和 **Agent Brief** 两个版本。
- **版本历史** — 报告会保留历史版本，API 暴露 latest 指针，前端新增 **Reports** 标签页可直接查看。
